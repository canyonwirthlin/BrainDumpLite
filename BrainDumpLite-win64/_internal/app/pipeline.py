...
Respond with valid JSON matching this exact schema — no markdown, no extra keys:
{
  ...
  "summary": "3-5 bullet points (each starting with •) covering every key action, decision, or emotion. First person. Be all-inclusive — miss nothing.",
  "items": [
    {
...
Deadlines (due_date_iso): resolve ANY stated or implied deadline to a concrete date
using the date lookup table provided ("by Friday" → that Friday's date, "by next week"
→ the coming Monday, "end of the month" → the last day of this month). Prefer the
EARLIER date when ambiguous. A deadline also means priority ≥ 4. Leave null when no
deadline is stated or implied — do NOT invent one.
