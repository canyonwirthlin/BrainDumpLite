...
from . import ai, db, pipeline, transcribe, updater
...
class SettingsIn(BaseModel):
    provider: str | None = None  # anthropic|openai|local|off
...
def item_ics(item_id: str):
    ...
    due = row["due_date"] or date.today().isoformat()
    start = due.replace("-", "")
    end = (date.fromisoformat(due) + timedelta(days=1)).strftime("%Y%m%d")
...
def put_settings(body: SettingsIn):
    if body.provider is not None:
        if body.provider not in ("anthropic", "openai", "local", "off"):
