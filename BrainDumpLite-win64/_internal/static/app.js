...
const PROVIDER_NAMES = { anthropic: "Claude", openai: "OpenAI", local: "Self-hosted", off: "AI off" };
...
function gcalUrl(t) {
  ...
  if (t.due_date) {
    ...
    const next = new Date(t.due_date + "T00:00");
...
function itemRow(it) {
  const overdue = it.due_date && it.due_date < todayIso() && !it.done;
  return `
    <div class="item ${it.status === "rejected" ? "rejected" : ""}" data-id="${it.id}">
      ...
      <div class="body">
        <div class="content">${esc(it.content)}
          ${it.due_date ? `<span class="chip ${overdue ? "overdue" : "due"}">⏰ ${fmtDay(it.due_date)}</span>` : ""}
...
function bindItemRows(container, items) {
...
async function renderTasks() {
  ...
  for (const t of tasks) {
    ...
    else if (t.due_date && t.due_date < today) groups.Overdue.push(t);
    ...
    <div class="card">${list.map((t) => `
      <div class="task-row ${t.done ? "done" : ""}" data-id="${t.id}">
        ...
        <div class="body grow">
          ...
          ${t.due_date ? `<span class="chip ${!t.done && t.due_date < today ? "overdue" : "due"}">⏰ ${fmtDay(t.due_date)}</span>` : ""}
